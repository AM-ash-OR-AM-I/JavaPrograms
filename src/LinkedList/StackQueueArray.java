...
class Stack extends BaseStack{
    ...
    public void push(int data) {
        if (top<(MAX_LENGTH-1))
            stack[++top] = data;
        else System.out.println("StackOverflow: Max length of stack exceeded.");
    }

    public void pop() {
        if (top==-1) System.out.println("Underflow, stack is empty");
        else System.out.println("Element removed = "+ stack[top--]);
    }
    ...
    public void displayStack() {
        for(int i=top; i>=0; i--){
            ...
            else System.out.printf("| %-5s |%n", stack[i]);
        }
    }
}
